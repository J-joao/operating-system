...
;
; FAT12 header
; orientation: https://wiki.osdev.org/FAT
;

; bpb required short jump
jmp short start
nop
...
bpb_fat_count:              db 2
bpp_dir_entries:            dw 0E0h
bpb_total_sectors:          dw 2880         ; 1.44MB
bpb_media_descriptor_type:  db 0F0h         ; floppy disk
...
ebr_volume_id:     db 33h, 01h, 33h, 01h    ; serial number
ebr_volume_label:  db "MACACO   OS"         ; 11 bytes, padded with spaces
...
wait_key_and_reboot:
    ...
    jmp 0FFFFh:0                ; force the computer to reboot

.halt:
    cli                         ; disable interrupts, so the CPU can't "unhalt"
    hlt
...
error_read_failed:      db "Read from disk failed!", 0x0D, 0x0A, 0
error_kernel_not_found: db "KERNEL.BIN file not found!", 0x0D, 0x0A, 0
